...
function App() {
  ...
  const downloadPdf = async () => {
    setExporting(true)
    try {
      pdfMake.createPdf(createPdfDefinition(locale)).download(`Diego_Minetti_Resume_${locale.toUpperCase()}.pdf`)
    } finally {
      setExporting(false)
    }
  }
    ...
    <div className="page">
      <header className="toolbar">
        ...
        <div className="actions">
          <button className="btn language-switch" type="button" onClick={() => setLocale(locale === 'en' ? 'es' : 'en')} aria-label={`Change language to ${t.switchLabel}`}>
            {t.switchLabel}
          </button>
          <a className="btn" href="https://github.com/DiegoMinetti" target="_blank" rel="noreferrer noopener">GitHub ↗</a>
          <a className="btn" href="https://www.linkedin.com/in/diegominetti" target="_blank" rel="noreferrer noopener">LinkedIn ↗</a>
      ...
      <main className="resume">
        <header className="resume__header">
          ...
          <p className="resume__contacts">
            {t.locationLine}<br />
            <a href="mailto:diegominetti@gmail.com">diegominetti@gmail.com</a><span className="sep">·</span>
            <a href="https://www.linkedin.com/in/diegominetti">linkedin.com/in/diegominetti</a><span className="sep">·</span>
            <a href="https://github.com/DiegoMinetti">github.com/DiegoMinetti</a>
          </p>
        </header>

        <Section title={t.profileTitle}><p>{t.profile}</p></Section>
        <Section title={t.expertiseTitle}><ul className="list columns">{t.expertise.map(item => <li key={item}>{item}</li>)}</ul></Section>
        <Section title={t.stackTitle}><div className="tags">{t.stack.map(item => <span className="tag" key={item}>{item}</span>)}</div></Section>
        <Section title={t.achievementsTitle}><ul className="list">{t.achievements.map(item => <li key={item}>{item}</li>)}</ul></Section>
        <Section title={t.experienceTitle}>
          {t.experience.map(item => <article className="experience-item" key={item.role}>
            <div className="exp-role">{item.role}</div><div className="exp-meta">{item.period}</div><div className="exp-desc">{item.description}</div>
            <ul className="list compact">{item.bullets.map(bullet => <li key={bullet}>{bullet}</li>)}</ul>
          </article>)}
        </Section>
        ...
        <Section title={t.educationTitle}><ul className="list">{t.education.map(item => <li key={item}>{item}</li>)}</ul></Section>
      </main>
...
function createPdfDefinition(locale: Locale): TDocumentDefinitions {
  ...
  return {
    ...
    info: { title: `Diego Minetti Resume ${locale.toUpperCase()}`, author: 'Diego Minetti', subject: t.title },
    ...
    content: [
      ...
      { canvas: [{ type: 'line', x1: 0, y1: 8, x2: 519, y2: 8, lineWidth: 0.5, lineColor: '#cbd5e1' }], margin: [0, 0, 0, 8] },
      ...section(t.profileTitle, { text: t.profile }),
      ...section(t.expertiseTitle, { columns: [{ ul: t.expertise.slice(0, 5) }, { ul: t.expertise.slice(5) }], columnGap: 22 }),
